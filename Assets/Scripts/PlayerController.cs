...
public class PlayerController : MonoBehaviour
{
    ...
    public List<FrameData> currentPlayerFrameDataList = new List<FrameData>();

    public int puppetTick = 0;
    ...
    private FrameData currentPhysicsFrameData;
    private PuppetData puppetData;
    ...
    public void Start() {
        ...
        GameMode.instance.onGameEndEvent = OnGameEnd;
        GameMode.instance.onGameStartEvent = OnGameStart;
        GameMode.instance.onRoundChangeEvent = OnRoundChange;
    }
    ...
    //Initialization of Puppet based on PuppetData
    public void InitPuppet(PuppetData puppetData)
    {
        this.SetIsPuppet(true);
        this.puppetData = puppetData;
        this.startPosition = this.puppetData.startPosition;
        this.playerPrefab = this.puppetData.playerPrefab;
    }

    public void OnRoundChange(PHASE oldPhase, PHASE newPhase)
    {
        if(newPhase == PHASE.DEMON)
        {
            CreatePuppet();
    ...
    //Executed Before Late Update and After FixedUpdate
    public void Update()
    {
        CheckAndDoAttack();
    }
    ...
    //FIXED UPDATE CAN GET EXECUTED MORE TIMES THAN UPDATE AND LATE UPDATE BE CAREFUL https://docs.unity3d.com/Manual/ExecutionOrder.html
    private void FixedUpdate()
    {
        //Create new frame data struct for current tick
        this.currentPhysicsFrameData = new FrameData();

        if (!isPuppet)
        {
            CheckAndDoMovement();

            //save position in frame data
            this.currentPhysicsFrameData.position = transform.position;

            //add Frame data
            this.currentPlayerFrameDataList.Add(currentPhysicsFrameData);
        } 
            else
        {   //DO puppet stuff


            //check if we are at or above the last frame
            if(this.puppetData.frameDataList.Count > this.puppetTick)
            {
                //Get current Frame data
                FrameData frameData = this.puppetData.frameDataList[puppetTick];

                //Set position by frame data of this tick
                this.transform.position = this.puppetData.frameDataList[puppetTick].position;

                if(frameData.isAttacking)
                {
                    this.AttackInDirection(frameData.attackDirection);
                }
            }
    ...
    public void AttackInDirection(Vector3 direction)
    {
        Debug.Log("Attack In Direction");
        // Attack stats
        ...
        if (!this.isPuppet)
        {
            // Save this attack in the latest physics frame data
            if (!this.isPuppet)
            {
                if (this.currentPlayerFrameDataList.Count != 0)
                {
                    FrameData latestPhysicsFrameData = this.currentPlayerFrameDataList[this.currentPlayerFrameDataList.Count - 1];
                    latestPhysicsFrameData.isAttacking = true;
                    latestPhysicsFrameData.attackDirection = direction;
                    this.currentPlayerFrameDataList[this.currentPlayerFrameDataList.Count - 1] = latestPhysicsFrameData;
                }
            }
    ...
    //Player method, which creates a puppet
    public void CreatePuppet()
    {
        GameObject newPlayerPuppet = GameObject.Instantiate(playerPrefab);
        newPlayerPuppet.transform.position = this.currentPlayerFrameDataList[0].position;
        PlayerController puppetController = newPlayerPuppet.GetComponent<PlayerController>();

        //construct puppet data for new player puppet
        PuppetData puppetData = new PuppetData();
        puppetData.playerPrefab = this.playerPrefab;
        puppetData.startPosition = this.startPosition;
        puppetData.frameDataList = new List<FrameData>();
        puppetData.frameDataList.AddRange(this.currentPlayerFrameDataList);

        //insert puppet data into new puppet player
        puppetController.InitPuppet(puppetData);

        //clear current frame data
        this.currentPlayerFrameDataList.Clear();
    }
